...
root_drive=$(lsblk -no pkname $(df / | tail -1 | awk '{print $1}'))
...
show_logo() {
	clear
	echo "   ______                 ";
	echo "  / ____/___ __   ______ _";
	echo " / /   / __ \`/ | / / __ \`/";
	echo "/ /___/ /_/ /| |/ / /_/ / ";
	echo "\\____/\\__,_/ |___/\\__,_/  ";
	echo "                          ";
	echo "			(C) Cava Contributors"
}

show_logo
echo " "
echo "Warning!"
echo "This code will wipe your data stored on your"
echo "drive. Please back your data up, you can't"
echo "restore it later."
...
echo "DESKTOP-${random}" > /etc/hostname
echo "root:${password}" | chpasswd
useradd -m -G wheel -s /bin/bash ${username}
echo "${username}:${password}" | sudo chpasswd

useradd -m -s /bin/bash aurbuilder
echo "aurbuilder ALL=(ALL) NOPASSWD: /usr/bin/pacman" > /etc/sudoers.d/aurbuilder && chmod 440 /etc/sudoers.d/aurbuilder

echo "%wheel ALL=(ALL:ALL) ALL" > /etc/sudoers.d/99-wheel && chmod 440 /etc/sudoers.d/99-wheel
...
HOME_URL="https://cava.chlorid.org"
DOCUMENTATION_URL="https://wiki.chlorid.org/
LOGO=archlinux-logo
OSREL
